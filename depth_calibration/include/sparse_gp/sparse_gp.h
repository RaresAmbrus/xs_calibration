...
private:
    ...
    void construct_covariance(Eigen::VectorXd& K, const Eigen::Vector3d &X, const Eigen::MatrixXd& Xv);
    void shuffle(std::vector<int>& ind, int n);
    void likelihood_dx(Eigen::Vector3d& dx, const Eigen::VectorXd& x, double y);
    void likelihood_dtheta(Eigen::VectorXd& dtheta, const Eigen::Vector3d &x, double y);
    double likelihood(const Eigen::Vector3d& x, double y);
public:
    void reset();
    ...
    sparse_gp(int capacity = 100, double s0 = 1e-2f);
};
