...
template <class Kernel, class Noise>
double sparse_gp<Kernel, Noise>::likelihood(const Vector3d& x, double y)
{
    //This is pretty much prediction
    //double kstar = kernel_function(x, x);
    double kstar = kernel.kernel_function(x, x);
...
template <class Kernel, class Noise>
void sparse_gp<Kernel, Noise>::likelihood_dtheta(VectorXd& dtheta, const Vector3d& x, double y)
{
...
template <class Kernel, class Noise>
void sparse_gp<Kernel, Noise>::construct_covariance(VectorXd& K, const Vector3d& X, const MatrixXd& Xv)
{
    K.resize(Xv.cols());
